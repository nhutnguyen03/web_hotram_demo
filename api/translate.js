...
export default async function handler(request, response) {
  ...
  try {
    ...
    for (const [language, targetLang] of Object.entries(languageCodes)) {
      ...
      if (!deeplResponse.ok) return response.status(502).json({ success: false, message: result.message || `DeepL không dịch được ngôn ngữ ${language}.` });
    ...
    return response.status(502).json({ success: false, message: 'Không thể kết nối DeepL. Kiểm tra API key và quota.' });
  }
}