export default async function handler(request, response) {
  ...
  try {
    ...
    return response.status(web3Response.ok && result.success ? 200 : 502).json({ success: Boolean(result.success), message: result.message || 'Web3Forms rejected the request.' });
  } catch {
